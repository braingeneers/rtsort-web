...
def subset_maxwell_h5(input_file, num_channels=8, num_seconds=5, output_file=None):
    ...
    try:
        ...
        print(
            f"Original file: {total_channels} channels, {total_samples} samples, {sampling_rate} Hz"
        )
        ...
        if num_samples > total_samples:
            ...
            print(
                f"Warning: Requested duration longer than available data. Using {total_samples/sampling_rate:.2f} seconds"
            )

        if num_channels > total_channels:
            num_channels = total_channels
            print(
                f"Warning: Requested channels exceed available channels. Using {total_channels} channels"
            )
    ...
    with h5py.File(original_file, "r") as orig_h5:
        with h5py.File(subset_file, "r") as subset_h5:
            ...
            assert np.array_equal(
                orig_samples, subset_samples
            ), "First samples don't match"

            # Try to load with SpikeInterface
            try:
                subset_recording = MaxwellRecordingExtractor(subset_file)
                print(
                    f"SpikeInterface validation - Channels: {subset_recording.get_num_channels()}"
                )
                print(
                    f"SpikeInterface validation - Sampling rate: {subset_recording.get_sampling_frequency()}"
                )
                print(
                    f"SpikeInterface validation - Samples: {subset_recording.get_num_samples()}"
                )
                print("✓ Subset file can be opened with SpikeInterface")
            except Exception as e:
                print(
                    f"✗ Warning: Subset file cannot be opened with SpikeInterface: {e}"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Create a subset of a Maxwell MEA h5 file"
    )
    ...
    parser.add_argument(
        "-o", "--output", help="Output file path (default: input_stem_subset.h5)"
    )
    parser.add_argument(
        "--validate",
        action="store_true",
