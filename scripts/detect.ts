...
class Float16Array {
  ...
  get(index: number): number {
    if (index < 0 || index >= this.length) {
      throw new Error(
        `Index ${index} out of bounds for Float16Array of length ${this.length}`
      );
    }
...
async function loadBinaryData(
  ...
  if (buffer.byteLength !== metadata.byte_size) {
    throw new Error(
      `File size mismatch: expected ${metadata.byte_size}, got ${buffer.byteLength}`
    );
  }
...
async function runInferenceScaling(
  ...
  for (let ch = 0; ch < numChannels; ch++) {
    for (let t = 0; t < pre_median_frames; t++) {
      window[ch * pre_median_frames + t] = scaledTraces.get(
        ch * timePoints + t
      );
    }
  }
  ...
  try {
    ...
    const windowTensor = new ort.Tensor("float16", windowUint16, [
      numChannels,
      pre_median_frames,
    ]);
    ...
    for (let ch = 0; ch < numChannels; ch++) {
      const channelWindow = window.slice(
        ch * pre_median_frames,
        (ch + 1) * pre_median_frames
      );
    ...
    const manualInferenceScaling =
      medianIqr !== 0 ? inference_scaling_numerator / medianIqr : 1;

    console.log(`Manual inference scaling factor: ${manualInferenceScaling}`);
    console.log(
      `Difference: ${Math.abs(inferenceScaling - manualInferenceScaling)}`
    );
...
async function runDetection(
  ...
  for (let ch = 0; ch < numChannels; ch++) {
    const channelData = windowData.slice(
      ch * sample_size,
      (ch + 1) * sample_size
    );
  ...
  try {
    ...
    const session = await ort.InferenceSession.create("models/detect.onnx");

    // Reshape for model input: [numChannels, 1, sample_size]
    const inputTensor = new ort.Tensor("float32", processedWindow, [
      numChannels,
      1,
      sample_size,
    ]);
    ...
    console.log(
      `Detection completed. Output shape: [${numChannels}, 1, ${num_output_locs}]`
    );
    console.log(
...
function compareOutputs(
  ...
  if (matchPercentage > 95) {
    console.log(
      `  ✅ PASS: ${matchPercentage.toFixed(
        2
      )}% of outputs match within tolerance`
    );
  } else {
    console.log(
      `  ❌ FAIL: Only ${matchPercentage.toFixed(
        2
      )}% of outputs match within tolerance`
    );
  }
...
async function main(): Promise<void> {
  try {
    ...
    console.log(
      `  Output locations: ${summary.model_parameters.num_output_locs}`
    );
    ...
    const tracesMetadata = await loadMetadata(
      `validation/${summary.files.scaled_traces.metadata}`
    );
    ...
    console.log(
      `Scaled traces loaded: shape [${tracesMetadata.shape.join(", ")}]`
    );

    // Load expected outputs for comparison
    const outputsMetadata = await loadMetadata(
      `validation/${summary.files.model_outputs.metadata}`
    );
    ...
    console.log(
      `Expected outputs loaded: shape [${outputsMetadata.shape.join(", ")}]`
    );
    console.log();
